...
function Start() {
    ...
    //Instantiate(EnemyPrefab, new Vector3(-1*startPositionX, 0f, startPositionZ),Quaternion.identity);
    //Instantiate(EnemyPrefab, new Vector3(-1*startPositionX, 0f, startPositionZ),Quaternion.identity);
...
function Update() {
    ...
    //Karakter.position+=Vector3(x,0,z);  not necessary anymore
    Karakter.transform.Translate(x * 0.2, 0f, z * 0.2);

    //	Karakter.transform.Translate(Input.acceleration.x*0.1f, 0f, Input.acceleration.z*0.1f);
...
//function checkBiggestAcceleration()
